...
import com.indialone.viewpager2andfragmentswithtablayout.databinding.FragmentAppleNewsBinding
...
class AppleNewsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val binding = FragmentAppleNewsBinding.inflate(inflater, container, false)

        newsViewModel = ViewModelProvider(this, ViewModelFactory()).get(NewsViewModel::class.java)

        newsViewModel.getAppleNews().observe(viewLifecycleOwner) { newsEntity ->
            binding.rvAppleNews.layoutManager = LinearLayoutManager(context)
            binding.rvAppleNews.adapter =
